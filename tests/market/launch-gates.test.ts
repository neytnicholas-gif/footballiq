...
describe('Player Market launch gates', () => {
  ...
  it('invalidates public catalogue caches after catalogue and gameweek writes', () => {
    ...
    expect(client).toContain('NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA')
    expect(client).toContain('/api/market/catalogue?release=')
  ...
  it('locks player trades from real kickoff until an exact-once fixture settlement', () => {
    const sql = read('supabase/migrations/20260816210000_beta_fixture_trade_safety.sql')
    const cutoffSql = read('supabase/migrations/20260816210500_enforce_valuation_cutoff.sql')
    const route = read('app/api/market/catalogue/route.ts')
    const client = read('lib/market/client.ts')
    expect(sql).toContain("valuation_eligible_from = timestamptz '2026-08-17 00:00:00+00'")
    expect(sql).toContain('candidate.kickoff_at <= p_at')
    expect(sql).toContain("candidate.status not in ('postponed', 'cancelled')")
    expect(sql).toContain('settlement.processed_at is null')
    expect(sql).toContain("raise exception 'PLAYER_TRADE_LOCKED'")
    expect(cutoffSql).toContain("raise exception 'FIXTURE_BEFORE_VALUATION_BOUNDARY'")
    expect(cutoffSql).toContain('before insert or update of fixture_date')
    expect(sql).toContain('primary key')
    expect(sql).toContain('provider_fixture_id text primary key')
    expect(sql).toContain('enable row level security')
    expect(route).toContain('is_trade_locked: Boolean(row.is_trade_locked)')
    expect(client).toContain('PLAYER_TRADE_LOCKED')
    expect(client).not.toContain('is_trade_locked: false')
  })
  ...
  it('refuses to publish a fallback-heavy Sportmonks opening price book', () => {
    ...
    const validation = read('lib/market/opening-price-validation.ts')
    ...
    expect(validation).toContain('fallback players exceed the conservative cap')
    expect(validation).toContain('positionAudit')
    ...
    expect(sync).toContain(".update(catalogueValues).eq('id', catalogueRow.id)")
    expect(sync).toContain("status: 'validated'")
  ...
  it('serializes catalogue writers and closes stale runs before safe retry', () => {
    const sql = read('supabase/migrations/20260816152758_prevent_overlapping_catalogue_syncs.sql')
    const sync = read('lib/market/server/catalogue-sync.ts')
    const gameweek = read('app/api/market/process-gameweek/route.ts')
    expect(sql).toContain("where run_type = 'catalogue_sync' and status = 'running'")
    expect(sql).toContain('create unique index if not exists market_processing_runs_one_catalogue_sync_idx')
    expect(sync).toContain("event: 'market.catalogue_sync.failed'")
    expect(sync).toContain('recoverLatestFailedCatalogueSync')
    expect(sync).toContain('retryCatalogueOperation(() => syncLeagueCatalogue(admin, catalogue), 1, 750)')
    expect(gameweek).toContain('recoverLatestFailedCatalogueSync()')
  })
  ...
  it('refreshes catalogue prices and skips processed player appearances without losing late fixture ratings', () => {
    ...
    expect(engine).toContain("select('id,provider_fixture_id,player:market_players!inner(provider_player_id)')")
    expect(engine).toContain(".lte('fixture_date', correctionWindowCutoff)")
    expect(engine).toContain(".order('id', { ascending: true })")
    expect(engine).toContain('.range(from, from + 999)')
    expect(engine).toContain('valuationEligibleFrom')
    expect(engine).toContain("admin.from('market_fixture_settlements').upsert")
  ...
  it('offers privacy-safe contextual beta feedback without hydration drift', () => {
    ...
    expect(feedback).toContain("topic: 'beta'")
    expect(feedback).toContain("const href = `/contact?")
    expect(feedback).not.toContain('window.navigator.userAgent')
