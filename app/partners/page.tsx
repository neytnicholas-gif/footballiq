...
export const metadata: Metadata = {
  title: 'Partner with Back Your Eye',
...
const opportunities = [
  ...
  { title: 'Content partner', description: 'Support an original Back Your Eye learning series. Editorial answers and game outcomes always stay independent.', icon: BarChart3 },
]
...
export default function PartnersPage() {
  const subject = encodeURIComponent('Back Your Eye partnership enquiry')
  return (
    <main className="min-h-screen bg-[#060b13] text-slate-100">
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-16">
        <div className="overflow-hidden rounded-[2rem] border border-white/12 bg-[radial-gradient(circle_at_12%_10%,rgba(16,185,129,.18),transparent_38%),linear-gradient(145deg,#0b1727,#08111e)] p-6 shadow-2xl sm:p-10">
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">Back Your Eye is building a free football-learning audience and a player-value game. We are opening a small number of honest, clearly labelled launch partnerships.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <a href={`mailto:${BRAND.supportEmail}?subject=${subject}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-400 px-5 py-3 font-bold text-slate-950 transition hover:bg-emerald-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200"><Mail className="size-4" /> Discuss a partnership</a>
            <Link href="/" className="inline-flex min-h-11 items-center rounded-xl border border-white/15 bg-white/5 px-5 py-3 font-bold transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300">Explore Back Your Eye</Link>
          </div>
        </div>
        ...
        <section className="mt-8 rounded-3xl border border-emerald-300/25 bg-emerald-400/10 p-6 text-center sm:p-8"><h2 className="text-2xl font-black">Interested in helping Back Your Eye launch?</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-300">Tell us about your brand, audience and idea. We will reply with suitable placements and current audience data.</p><a href={`mailto:${BRAND.supportEmail}?subject=${subject}`} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-400 px-5 py-3 font-bold text-slate-950 transition hover:bg-emerald-300"><Mail className="size-4" /> {BRAND.supportEmail}</a></section>
      </section>
    </main>
  )
}
