...
import { CheckCircle2, Dices, Search, Sparkles } from 'lucide-react'
...
export default function FootballDuelsPage() {
  ...
  function markComplete(packId: string, score: number) {
    if (packId.startsWith('quick-mix')) return
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary"><Sparkles className="size-3.5" /> Football Duels 2.0</div>
            <h1 className="mt-4 max-w-3xl text-4xl font-bold tracking-tight sm:text-6xl">One stat. Two legends. <span className="text-primary">Make the call.</span></h1>
            <p className="mt-4 max-w-2xl text-lg text-muted-foreground">Beat the clock, build combos and chase a perfect 10. Ties are real — “Same” can be the correct answer.</p>
          </div>
          <button onClick={selectRandom} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-primary px-6 font-semibold text-primary-foreground"><Dices className="size-5" /> Surprise me</button>
        </div>

        <div className="mt-8"><DuelQuiz key={selected.id} pack={selected} onComplete={markComplete} /></div>

        <section className="mt-14 border-t border-border pt-10">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div><p className="text-xs font-semibold uppercase tracking-[.25em] text-primary">Choose a pack</p><h2 className="mt-2 text-3xl font-bold">Master every category</h2><p className="mt-2 text-muted-foreground">Your best score is saved on this device. Replays shuffle the order and sides.</p></div>
            <div className="rounded-full border border-border bg-card px-4 py-2 text-sm"><strong className="text-primary">{Object.keys(completed).length}</strong>/{duelPacks.length} packs played</div>
          </div>
          ...
          {filtered.length === 0 && <div className="mt-6 rounded-3xl border border-dashed border-border p-10 text-center text-muted-foreground">No packs match that search.</div>}
        </section>
      </section>
    </main>
  )
