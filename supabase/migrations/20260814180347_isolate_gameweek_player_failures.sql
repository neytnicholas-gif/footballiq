...
-- Keep valid player updates when one provider row causes an unexpected SQL error.
-- Each player runs in its own PL/pgSQL subtransaction; a failed item rolls back
-- only its own writes and is reported to operations in the function result.
...
begin
  ...
  if processed=0 then
    ...
    return jsonb_build_object('ok',true,'unchanged',true,'gameweek_id',g.id,'processed_players',0,
      'skipped_players',skipped,'fixture_count',0,'failed_items',failed_items);
  end if;
