...
alter table public.market_gameweeks
  add column if not exists gameweek_type text not null default 'transfer'
  check (gameweek_type in ('transfer','results'));

update public.market_gameweeks
set gameweek_type = case when gameweek_key like 'sportmonks-%' then 'results' else 'transfer' end;

create index if not exists market_gameweeks_type_state_dates_idx
  on public.market_gameweeks(gameweek_type,state,opens_at desc,closes_at desc);
...
as $$
  ...
  where g.gameweek_type='transfer' and now() >= g.opens_at and now() < g.closes_at and g.state in ('open','revealed')
...
begin
  ...
  for item in select value from jsonb_array_elements(p_updates) loop
    ...
    movement:=greatest(-300000,least(300000,round((rolling_rating-baseline)/0.22)*100000));
    ...
    insert into public.market_valuation_events(
      ...
      mp.performance_bank_milli,movement,
      'Verified Sportmonks rating; rolling form from '||appearances_used||' appearance(s)',
      'fiq-real-performance-v2.0.0',(item->>'fixture_date')::timestamptz,
      'sportmonks:'||(item->>'provider_fixture_id')||':'||(item->>'provider_player_id')
    );
    update public.market_players set current_price_minor=new_price,
  ...
  select p.id,g.id,b.total_minor,p.total_portfolio_value_minor,p.cash_balance_minor,p.current_holdings_value_minor,
    ...
    coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'explanation','Verified ratings updated this value using five-match rolling form.') order by player.display_name)
...
as $$
with mine as (
  select r.*,g.week_number,g.label,p.user_id
  from public.market_gameweek_reveals r
  join public.market_portfolios p on p.id=r.portfolio_id
  join public.market_gameweeks g on g.id=r.gameweek_id
  where p.user_id=(select auth.uid())
  order by r.created_at desc limit greatest(1,least(p_limit,52))
), enriched as (
  select mine.*,
    (select value from jsonb_array_elements(mine.holding_movements) value order by (value->>'delta')::integer desc limit 1) winner,
    (select value from jsonb_array_elements(mine.holding_movements) value order by (value->>'delta')::integer asc limit 1) loser
  from mine
)
select coalesce(jsonb_agg(jsonb_build_object(
  'scope_key',user_id::text,'week_number',week_number,'week_label',label,'completed_at',created_at,
  'previous_portfolio_value',previous_portfolio_value_minor,'new_portfolio_value',new_portfolio_value_minor,
  'weekly_change',weekly_change_minor,
  'weekly_return_pct',round((weekly_change_minor::numeric/nullif(previous_portfolio_value_minor,0))*100,2),
  'cash_after',cash_after_minor,'invested_after',invested_after_minor,'total_after',new_portfolio_value_minor,
  'biggest_winner',coalesce(winner,jsonb_build_object('player_id',null,'player_name',null,'delta',0)),
  'biggest_loser',coalesce(loser,jsonb_build_object('player_id',null,'player_name',null,'delta',0)),
  'best_held_player',coalesce(winner,jsonb_build_object('player_id',null,'player_name',null,'delta',0)),
  'weakest_held_player',coalesce(loser,jsonb_build_object('player_id',null,'player_name',null,'delta',0)),
  'holdings',holding_movements
) order by created_at desc),'[]'::jsonb) from enriched
$$;
