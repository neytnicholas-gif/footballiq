...
import { fetchSportmonksCompletedGameweek } from '@/lib/market/server/sportmonks-client'
...
export async function processLatestVerifiedGameweek() {
  const gameweek = await fetchSportmonksCompletedGameweek()
  const admin = adminClient()
  const runKey = `verified-gameweek:${gameweek.gameweekKey}`
  const { data: existing } = await admin.from('market_processing_runs').select('status,report').eq('run_key', runKey).maybeSingle()
  if (existing?.status === 'completed') return { unchanged: true, ...(existing.report as Record<string, unknown>) }
