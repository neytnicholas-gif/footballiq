...
export function PredictionsGame() {
  ...
  const leagueActionDialogRef = useModalFocus<HTMLElement>({
    open: Boolean(leagueAction),
    onClose: () => setLeagueAction(null),
    canClose: !working,
  })
  ...
  return (
    <div className="space-y-5">
      ...
      {leagueAction ? <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/80 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) setLeagueAction(null) }}><section ref={leagueActionDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="prediction-league-action-title" aria-describedby="prediction-league-action-description" className="w-full max-w-md rounded-[1.75rem] border border-slate-600 bg-slate-900 p-5 shadow-2xl outline-none sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-sky-300">Confirm change</p><h3 id="prediction-league-action-title" className="mt-1 text-2xl font-black text-white">{leagueAction.kind === 'delete' ? 'Delete this league?' : 'Leave this league?'}</h3></div><button type="button" aria-label="Close confirmation" disabled={working} onClick={() => setLeagueAction(null)} className="grid size-10 place-items-center rounded-xl border border-slate-700 text-slate-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-300"><X className="size-4" /></button></div><p id="prediction-league-action-description" className="mt-3 text-sm leading-6 text-slate-300">{leagueAction.kind === 'delete' ? <><strong className="text-white">{leagueAction.league.name}</strong> and its private table will disappear for every member. Everyone&apos;s saved predictions remain on their profile.</> : <>You will leave <strong className="text-white">{leagueAction.league.name}</strong>. Your saved predictions remain on your profile.</>}</p><div className="mt-5 grid gap-2 sm:grid-cols-2"><button autoFocus type="button" disabled={working} onClick={() => setLeagueAction(null)} className="min-h-11 rounded-xl border border-slate-600 font-bold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-300">Keep league</button><button type="button" disabled={working} onClick={() => void completeLeagueAction()} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl font-black text-slate-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-300 disabled:opacity-50 ${leagueAction.kind === 'delete' ? 'bg-rose-300' : 'bg-sky-300'}`}>{working ? <Loader2 className="size-4 animate-spin" /> : null}{leagueAction.kind === 'delete' ? 'Delete for everyone' : 'Leave league'}</button></div></section></div> : null}
    </div>
  )
}
