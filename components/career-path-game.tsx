...
export function CareerPathGame() {
  ...
  async function save() {
    ...
    const { error } = await saveQuizResult({
      ...
      xp: 20 + score * 10,
      completionKey: buildCompletionKey('career-path-1', runKey),
    })
    if (!error) {
      setSaved(true)
