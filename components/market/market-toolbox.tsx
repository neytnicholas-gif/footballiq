...
export function MarketToolbox() {
  ...
  return <div className="space-y-6">
    ...
    {message?<p role="status" className="rounded-xl border border-emerald-600/25 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-950">{message}</p>:null}{error?<p role="alert" className="rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm font-bold text-destructive">{error}</p>:null}
    ...
    <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-primary">Permanent upgrade catalogue</p><h2 className="mt-2 text-3xl font-black">Build your Clubhouse</h2><p className="mt-2 text-sm text-muted-foreground">Cheap tools help early. Larger tools reward a longer Market journey.</p></div><Link href="/market/rewards#reward-shop" className="rounded-xl bg-primary px-4 py-2.5 text-sm font-black text-primary-foreground">Open reward shop</Link></div>
      <div className="mt-4 rounded-xl bg-secondary/50 p-4 text-sm"><strong>How to get one:</strong> complete challenges to earn badges and Style Credits. Reach the shown tier, finish its trade and Reveal goals, then claim it permanently in Rewards. VX is only for your players.</div>
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{toolKeys.map(([key,name,copy])=>{const item=progression.store.find((entry)=>entry.item_key===key);const tier=item?clubhouseTierForItem(item):null;return <div key={key} className={`rounded-2xl border p-4 ${owns(key)?'border-emerald-500/30 bg-emerald-50':'border-border bg-background/70'}`}><p className="flex items-center gap-2 font-black">{owns(key)?<Check className="size-4 text-emerald-700"/>:<LockKeyhole className="size-4 text-muted-foreground"/>}{name}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{copy}</p>{item?<div className="mt-3 space-y-1 text-xs text-muted-foreground"><p><strong className="text-foreground">{tier?.name}</strong> · {item.required_badges} badges</p><p>{item.required_trades} trades · {item.required_reveals} Reveals</p><p className="font-black text-primary">{item.price_credits} Style Credits</p></div>:null}<p className="mt-3 text-xs font-bold">{owns(key)?'Owned forever':'Buy in Rewards when eligible'}</p></div>})}</div>
    </section>
...
function PlanMetric({label,value,warn=false}:{label:string;value:string;warn?:boolean}){return <div className={`rounded-xl border p-3 ${warn?'border-rose-300 bg-rose-50':'border-border bg-background/70'}`}><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-black">{value}</p></div>}
