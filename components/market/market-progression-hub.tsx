...
export function MarketProgressionHub() {
  ...
  return <div className="space-y-5">
    <section className="rounded-[2rem] border border-emerald-950/10 bg-card p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3"><SimpleMetric label="Badges" value={`${completed.length}/${progression.challenges.length}`}/><SimpleMetric label="Clubhouse level" value={currentTier.name}/><SimpleMetric label="Market activity" value={`${progression.trade_count} trades · ${progression.reveal_count} Reveals`}/></div>
    ...
    {closest?<section aria-labelledby="next-goal-title" className="rounded-[2rem] border border-emerald-500/25 bg-gradient-to-r from-emerald-50 to-white p-6 sm:p-7"><div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-center"><div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-800">Your next goal</p><h2 id="next-goal-title" className="mt-2 text-2xl font-black">{closest.title}</h2><p className="mt-1 text-sm text-muted-foreground">{closest.description}</p><ChallengeProgress challenge={closest} large/></div><div className="rounded-2xl border border-emerald-700/15 bg-white px-5 py-4 md:min-w-52"><p className="text-xs font-bold text-muted-foreground">You will earn</p><p className="mt-1 font-black text-emerald-900">{closest.badge_name}</p><p className="mt-1 text-sm font-bold text-primary">+{closest.reward_credits} Style Credits</p></div></div></section>:null}
    ...
    {view==='challenges'?<section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-primary">Challenges</p><h2 className="mt-2 text-3xl font-black">Choose a goal</h2><p className="mt-2 text-sm text-muted-foreground">Your progress updates automatically when you play.</p></div><div role="group" aria-label="Filter challenges" className="flex rounded-xl border border-border bg-background p-1"><FilterButton active={filter==='next'} onClick={()=>{setFilter('next');setShowAllChallenges(false)}}>To do ({progression.challenges.length-completed.length})</FilterButton><FilterButton active={filter==='earned'} onClick={()=>{setFilter('earned');setShowAllChallenges(false)}}>Earned ({completed.length})</FilterButton><FilterButton active={filter==='all'} onClick={()=>{setFilter('all');setShowAllChallenges(false)}}>All</FilterButton></div></div>
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{challengesToShow.map((challenge)=><ChallengeCard key={challenge.challenge_key} challenge={challenge} busy={Boolean(busy)} showcasedCount={showcased.length} badgeLimit={badgeLimit} onToggleShowcase={toggleShowcase}/>)}</div>
    ...
    {view==='shop'?<section id="reward-shop" className="scroll-mt-24 rounded-[2rem] border border-border bg-card p-6 sm:p-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rewardsToShow.map((item)=><RewardCard key={item.item_key} item={item} progression={progression} isEquipped={equipped.has(item.item_key)} busy={Boolean(busy)} onAct={act}/>)}</div>
...
function challengeTone(iconKey:string){
  if(['users','shield'].includes(iconKey))return{card:'border-sky-200/80 bg-sky-50/35',bar:'bg-sky-400',icon:'bg-sky-100 text-sky-800',text:'text-sky-800'}
  if(['trending-up','chart'].includes(iconKey))return{card:'border-amber-200/80 bg-amber-50/35',bar:'bg-amber-400',icon:'bg-amber-100 text-amber-800',text:'text-amber-800'}
  if(['calendar','sparkles'].includes(iconKey))return{card:'border-violet-200/80 bg-violet-50/35',bar:'bg-violet-400',icon:'bg-violet-100 text-violet-800',text:'text-violet-800'}
  if(['globe','eye'].includes(iconKey))return{card:'border-rose-200/80 bg-rose-50/35',bar:'bg-rose-400',icon:'bg-rose-100 text-rose-800',text:'text-rose-800'}
  if(['trophy','crown','swords'].includes(iconKey))return{card:'border-indigo-200/80 bg-indigo-50/35',bar:'bg-indigo-400',icon:'bg-indigo-100 text-indigo-800',text:'text-indigo-800'}
  return{card:'border-teal-200/80 bg-teal-50/30',bar:'bg-teal-400',icon:'bg-teal-100 text-teal-800',text:'text-teal-800'}
}

function rewardTone(type:MarketRewardItem['item_type']){
  if(type==='formation')return{card:'border-sky-200/80 bg-sky-50/30',bar:'bg-sky-400',icon:'bg-sky-100 text-sky-800',text:'text-sky-800'}
  if(type==='utility')return{card:'border-amber-200/80 bg-amber-50/30',bar:'bg-amber-400',icon:'bg-amber-100 text-amber-800',text:'text-amber-800'}
  if(type==='access')return{card:'border-indigo-200/80 bg-indigo-50/30',bar:'bg-indigo-400',icon:'bg-indigo-100 text-indigo-800',text:'text-indigo-800'}
  if(type==='title')return{card:'border-violet-200/80 bg-violet-50/30',bar:'bg-violet-400',icon:'bg-violet-100 text-violet-800',text:'text-violet-800'}
  return{card:'border-rose-200/80 bg-rose-50/25',bar:'bg-rose-300',icon:'bg-rose-100 text-rose-800',text:'text-rose-800'}
}
