...
function ChallengeCard({challenge,busy,showcasedCount,badgeLimit,onToggleShowcase}:{challenge:MarketChallenge;busy:boolean;showcasedCount:number;badgeLimit:number;onToggleShowcase:(key:string)=>Promise<void>}){
  ...
  return <article className={`flex flex-col rounded-2xl border p-5 ${done?'border-emerald-500/30 bg-emerald-50/70':'border-border bg-background/70'}`}><div className="flex items-start justify-between gap-3"><span className={`rounded-xl p-2.5 ${done?'bg-emerald-600 text-white':'bg-secondary text-foreground'}`}><Icon className="size-5"/></span><span className="rounded-full border border-border bg-card px-2.5 py-1 text-xs font-bold">+{challenge.reward_credits}</span></div><h3 className="mt-4 text-lg font-black">{challenge.title}</h3><p className="mt-1 flex-1 text-sm leading-5 text-muted-foreground">{challenge.description}</p><ChallengeProgress challenge={challenge}/><p className="mt-3 text-sm font-bold">{done?<span className="text-emerald-800"><Check className="mr-1 inline size-4"/>Earned: {challenge.badge_name}</span>:`Badge: ${challenge.badge_name}`}</p>{done?<button disabled={busy} onClick={()=>void onToggleShowcase(challenge.challenge_key)} className="mt-4 min-h-11 w-full rounded-xl border border-emerald-700/25 bg-white px-3 py-2 text-sm font-bold text-emerald-950 disabled:opacity-50">{challenge.showcased?'Remove from profile':showcasedCount>=badgeLimit?'Swap into profile':'Show on profile'}</button>:null}</article>
}
