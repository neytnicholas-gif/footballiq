  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
      <div className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
        ...
        {loading ? <p className="mt-5 text-sm text-muted-foreground">Checking your Arena status…</p> : arena.queue ? <div className="mt-5 rounded-2xl border border-amber-300/20 bg-[#3a3016] p-5"><p className="flex items-center gap-2 font-black text-amber-100"><Clock3 className="size-5"/>Searching for an opponent</p><p className="mt-1 text-sm text-amber-50/70">Queued for {arena.queue.gameweek_label}. The search widens gradually if nobody near your rating is available.</p><button disabled={busy} onClick={()=>void cancel()} className="mt-4 min-h-11 rounded-xl border border-amber-200/20 bg-amber-200 px-4 py-2 text-sm font-bold text-amber-950 disabled:opacity-50">Leave queue</button></div>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-indigo-300/20 bg-[radial-gradient(circle_at_top_right,rgba(129,140,248,.2),transparent_45%),linear-gradient(145deg,rgba(16,26,46,.98),rgba(9,24,34,.98))] p-6 shadow-xl shadow-black/15 sm:p-8"><ShieldCheck className="size-8 text-indigo-300"/><h2 className="mt-4 text-2xl font-black">Clear rules. No surprises.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">The database uses these exact rules for every player.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Rule icon={<Gauge className="size-5"/>} title="Closest skill first" copy="The search starts within 200 rating points. It widens by 25 each minute and stops at 500."/><Rule icon={<Trophy className="size-5"/>} title="One fair score" copy="Your XI’s percentage value change is compared with your opponent’s. The higher return wins."/><Rule icon={<Coins className="size-5"/>} title="Credits for everyone" copy="Win: 75. Draw: 35 each. Loss: 15. Your Market Credits and Style Credits are never at risk."/><Rule icon={<Sparkles className="size-5"/>} title="Skill moves after results" copy="A result can move your skill rating by up to 32 points. Beating a stronger opponent moves it more."/></div></div>
    </section>

    <section className="rounded-[2rem] border border-amber-300/20 bg-[radial-gradient(circle_at_90%_10%,rgba(251,191,36,.15),transparent_30%),linear-gradient(135deg,#332b17,#172e2a)] p-6 text-amber-50 sm:p-8"><div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-xs font-black uppercase tracking-[.22em] text-amber-300">When does it start?</p><h2 className="mt-2 text-2xl font-black text-white">Queue during an open gameweek.</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-amber-50/70">Join once your XI is full. The matchup locks to that gameweek, then settles after both players receive The Reveal. You can prepare before the first league matches and come back after the final result is processed.</p></div><Link href="/market/leagues" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-amber-200 px-5 py-2.5 text-sm font-black text-amber-950">Create a friends league</Link></div></section>
...
function Rule({icon,title,copy}:{icon:ReactNode;title:string;copy:string}){return <div className="rounded-2xl border border-indigo-200/15 bg-white/[.055] p-4"><span className="grid size-10 place-items-center rounded-xl bg-indigo-300/15 text-indigo-200">{icon}</span><p className="mt-3 font-black text-white">{title}</p><p className="mt-1 text-xs leading-5 text-indigo-100/60">{copy}</p></div>}
