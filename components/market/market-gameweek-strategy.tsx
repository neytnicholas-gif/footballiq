...
import {
  MARKET_GAMEWEEK_CHIPS, MARKET_OUTFIELD_POSITIONS, marketChipDefinition,
      ...
      <div className="border-b border-white/10 p-5 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-2xl">
            ...
            <h2 id="gameweek-strategy-title" className="mt-2 text-2xl font-black text-white sm:text-3xl">Choose your shape. Play one chip.</h2>
            <p className="mt-2 text-sm leading-6 text-slate-300">Your formation decides the 11 places you can fill. Your chip changes how this week’s verified match movement affects your held copies.</p>
          </div>
      ...
      <div className="p-5 sm:p-7">
        ...
        {!userSignedIn ? (
          ...
          <div className={`mt-4 rounded-2xl border p-5 ${CHIP_TONES[activeChip.tone].card}`}>
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-slate-300">Chip locked in</p><h4 className="mt-1 text-2xl font-black text-white">{activeChip.name}</h4><p className="mt-1 text-sm text-slate-300">{activeChip.summary}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${CHIP_TONES[activeChip.tone].badge}`}>{activeChip.multiplierLabel}</span></div>
            <p className="mt-4 text-sm font-bold text-white">Target: {chipStatus.active_chip.target_position ? MARKET_OUTFIELD_POSITIONS.find((position) => position.key === chipStatus.active_chip?.target_position)?.label : chipStatus.active_chip.targets.map((target) => target.player_name).join(', ')}</p>
