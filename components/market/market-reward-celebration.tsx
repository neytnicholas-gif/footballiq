...
export function MarketRewardCelebration() {
  ...
  useEffect(()=>{if(!earned.length)return;const onKey=(event:KeyboardEvent)=>{if(event.key==='Escape')close()};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey)},[close,earned.length])
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-emerald-950/80 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="reward-celebration-title">
    <div className="relative my-6 w-full max-w-xl overflow-hidden rounded-[2rem] border border-white/20 bg-[radial-gradient(circle_at_50%_10%,rgba(251,191,36,.34),transparent_36%),linear-gradient(145deg,#0b2f27,#071b17)] p-1 shadow-[0_32px_100px_rgba(0,0,0,.55)] motion-safe:animate-in motion-safe:zoom-in-95 motion-safe:fade-in motion-reduce:animate-none">
      ...
      <div className="relative rounded-[1.75rem] border border-white/10 bg-white/[.06] px-6 pb-7 pt-12 text-center text-white sm:px-10 sm:pb-10 sm:pt-14">
        <button type="button" onClick={close} aria-label="Skip reward celebration" className="absolute right-4 top-4 inline-flex min-h-9 items-center gap-1 rounded-full px-3 text-xs font-bold text-white/60 transition hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-300">Skip<X className="size-3.5"/></button>
        ...
        <button type="button" disabled={saving} onClick={()=>void alwaysSkip()} className="mt-5 text-xs font-semibold text-white/45 underline decoration-white/20 underline-offset-4 hover:text-white/80 disabled:opacity-50">{saving?'Saving preference...':'Always skip reward celebrations'}</button>
      </div>
    </div>
  </div>
}
