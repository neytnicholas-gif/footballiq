...
export function CompetitiveLeaderboard({ initialBoard = 'overall' }: { initialBoard?: string }) {
  ...
  return <div>
    ...
    <section className="mt-8 overflow-hidden rounded-[2rem] border border-border bg-card">
      ...
      {!loading && !error && players.length === 1 ? <div className="border-b border-border bg-primary/[.06] px-6 py-4"><p className="text-sm font-semibold text-foreground">The table is just getting started.</p><p className="mt-1 text-sm text-muted-foreground">You set the early score. Invite a friend and give yourself someone to chase.</p></div> : null}
